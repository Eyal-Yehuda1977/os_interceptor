...
#define __debug_mode_ssh_enable_(tcph)					\
	if (tcph->source == ntohs(22) || tcph->dest == ntohs(22) )	\
		return NF_ACCEPT;
                        ...
                        char *err) 
{
	...
	if (*daddr == 0) {
		snprintf(err, 
			 CONTROL_BUFFER_DEFAULT_SIZE,
			 "invalid server ip address: daddr = [ %u ] ", 
			 *daddr);
	...
	if (*saddr == 0) {
		snprintf(((err + strlen(err))-1), 
			 CONTROL_BUFFER_DEFAULT_SIZE,
			 " invalid agent ip address: saddr  = [ %u ] ",
			 *saddr);
...
int __quarantine_mode(void) { return __quarantine_mode_; }
	...
	*/
	...
	case IPPROTO_ICMP: 
		...
		if( __quarantine_mode_ )
			...
			ret = NF_ACCEPT;  
	break;
	case IPPROTO_UDP:  ret = NF_ACCEPT;  
	break; 
	default:           ret = NF_ACCEPT;  
	break;
	}
...
{ 
	...
	if (ret == ERROR) {
		...
		snprintf(err,
			 CONTROL_BUFFER_DEFAULT_SIZE,
			 "ip addres used by agent [ %s ]"		\
			 " is not related to any network interface. ",
			 ip_buf);
	}
